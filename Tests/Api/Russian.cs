...
namespace Tests.Api {
	[TestFixture]
	internal sealed class Russian {
        private Meduza.net.Api _api;
	    
        [SetUp]
	    public void Init() {
            _api = new Meduza.net.Api(Language.Russian);
	    }
		[Test]
		...
		public async void NonDefaultInitialization() {
            var api = new Meduza.net.Api(Language.Russian, false);
		...
		public async void LoadCard() {
			var uri = _api.Main.Root.First(r => r.ScreenType == ScreenType.Cards).Collection.First();
			var document = await _api.LoadAsync(uri);
